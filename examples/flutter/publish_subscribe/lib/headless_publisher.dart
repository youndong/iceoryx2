...
class HeadlessPublisher {
  ...
  void _printStats() {
    if (_startTime != null) {
      ...
      print('[Stats] Total: $_messageCounter, Success: $_successCount, Failed: $_failureCount, Rate: ${rate.toStringAsFixed(2)} msg/s');
    }
  }
  ...
  void _printFinalStats() {
    ...
    print('Success rate: ${(_successCount / _messageCounter * 100).toStringAsFixed(2)}%');
    if (_startTime != null) {
      final duration = DateTime.now().difference(_startTime!);
      final rate = _messageCounter / duration.inSeconds;
